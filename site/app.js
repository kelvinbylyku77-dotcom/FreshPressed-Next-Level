...
function renderShop(updateURL=false){
  ...
  let list=products.filter(p=>(shopState.cat==='all'||p.cat===shopState.cat)&&(!shopState.query||`${p.name} ${p.desc}`.toLocaleLowerCase().includes(shopState.query.toLocaleLowerCase())));
...
function applyMotion(){
  ...
  $('#motionToggle').textContent=motionPaused?'Enable motion':'Pause motion';$('#motionToggle').setAttribute('aria-pressed',String(motionPaused));
  syncVideos();observeReveals();
}
$('#motionToggle').addEventListener('click',()=>{motionPaused=!motionPaused;writeStore('fp-v5-motion',motionPaused);applyMotion();});
